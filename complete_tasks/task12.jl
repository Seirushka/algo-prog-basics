...
function main!(robot)
    ...
    count::Int, flag::Int, side = 0, 0, Ost
...
function full_move_bord_check!(robot, flag, side, bord_side)
    ...
    while !isborder(robot, side)
        if flag == 0
            isborder(robot, bord_side) && (count_i +=1; flag = 1)
        elseif flag == 1 
            !isborder(robot, bord_side) && (flag = 2)
        else
            isborder(robot, bord_side) && (flag = 1)
            !isborder(robot, bord_side) && (flag = 0)
        end
        move!(robot, side)
    end
    return count_i
